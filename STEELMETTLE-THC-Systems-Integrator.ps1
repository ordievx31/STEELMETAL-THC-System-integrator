...
function Check-For-LocalWorkspaceUpdate {
    $u = Get-UpdateConfig
    if (-not $u.enabled) {
        return @{ status = 'disabled'; updateAvailable = $false; message = 'Updates disabled' }
    }

    if ($u.mode -ne 'local-workspace') {
        return @{ status = 'not-local-mode'; updateAvailable = $false; message = 'Not in local workspace mode' }
    }
    ...
    if ([string]::IsNullOrWhiteSpace($workspaceDir)) {
        ...
        $workspaceDir = if ([System.IO.Path]::IsPathRooted($workspaceDir)) {
            $workspaceDir
        } else {
            Join-Path $baseDir $workspaceDir
        }
    }

    if (-not (Test-Path $workspaceDir -PathType Container)) {
        return @{ status = 'workspace-not-found'; updateAvailable = $false; message = "Workspace dir not found: $workspaceDir" }
    }

    try {
        $criticalFiles = @(
            'config.json',
            'STEELMETTLE-THC-Systems-Integrator.ps1',
            'MachMacroInstaller.ps1'
        )

        # Calculate current file hashes
        $currentHashes = @{}
        foreach ($file in $criticalFiles) {
            $sourcePath = Join-Path $workspaceDir $file
            if (Test-Path $sourcePath) {
                try {
                    $hash = (Get-FileHash -Path $sourcePath -Algorithm SHA256).Hash
                    $currentHashes[$file] = $hash
                } catch {
                    $currentHashes[$file] = 'error'
                }
            }
        }

        # Load cached hashes
        $cacheFile = if ([string]::IsNullOrWhiteSpace($u.cacheHashFile)) {
            Join-Path $baseDir 'logs' 'workspace_file_hashes.json'
        } else {
            if ([System.IO.Path]::IsPathRooted($u.cacheHashFile)) {
                $u.cacheHashFile
            } else {
                Join-Path $baseDir $u.cacheHashFile
            }
        }
        ...
        if (Test-Path $cacheFile) {
            try {
                $cachedContent = Get-Content -Path $cacheFile -Raw
                $cachedHashes = ConvertFrom-JsonCompat $cachedContent
                if (-not $cachedHashes -is [hashtable]) { $cachedHashes = @{} }
            } catch {
                $cachedHashes = @{}
            }
        }
        ...
        # Store current hashes for next check
        try {
            $logsDir = Split-Path $cacheFile
            if (-not (Test-Path $logsDir)) {
                New-Item -ItemType Directory -Path $logsDir -Force | Out-Null
            }
            $currentHashes | ConvertTo-Json | Set-Content -Path $cacheFile -Force
        } catch {
        ...
        return @{
            ...
            message = if ($hasUpdates) { 'Workspace files have been modified' } else { 'Workspace is up to date' }
            workspaceDir = $workspaceDir
        }
    } catch {
        return @{ status = 'error'; updateAvailable = $false; message = $_.Exception.Message }
    }
}

function Sync-WorkspaceToInstalled {
    param(
        [string]$WorkspaceDir,
        [string]$InstalledDir
    )

    if ([string]::IsNullOrWhiteSpace($WorkspaceDir)) { $WorkspaceDir = $baseDir }
    if ([string]::IsNullOrWhiteSpace($InstalledDir)) { $InstalledDir = $baseDir }

    if (-not (Test-Path $WorkspaceDir)) {
        return @{ success = $false; message = "Workspace path not found: $WorkspaceDir" }
    }
    ...
    try {
        ...
        foreach ($item in $filesToCopy) {
            $sourcePath = Join-Path $WorkspaceDir $item
            $targetPath = Join-Path $InstalledDir $item
        ...
        return @{
            success = $true
            message = "Synced $copiedCount items from workspace to installed app"
...
function Prompt-And-RunLocalWorkspaceUpdate {
    $checkResult = Check-For-LocalWorkspaceUpdate
    ...
    $u = Get-UpdateConfig
    if ($u.autoSync) {
        Set-UiProgress 'Updating from workspace...' 50

        $syncResult = Sync-WorkspaceToInstalled -WorkspaceDir $checkResult.workspaceDir -InstalledDir $baseDir

        if ($syncResult.success) {
            Set-UiProgress "$($syncResult.message)" 90
            [System.Windows.Forms.MessageBox]::Show(
                "App has been updated from your workspace.`n`n$($syncResult.message)`n`nThe app will restart now to apply changes.",
                'Update Applied',
                [System.Windows.Forms.MessageBoxButtons]::OK,
                [System.Windows.Forms.MessageBoxIcon]::Information
            ) | Out-Null
            ...
            $form.Close()
            return @{ success = $true; message = 'Restarting app'; updated = $true }
        } else {
            Set-UiProgress 'Update sync failed.' 100
            [System.Windows.Forms.MessageBox]::Show(
                "Could not sync workspace changes:`n`n$($syncResult.message)",
                'Update Failed',
                [System.Windows.Forms.MessageBoxButtons]::OK,
                [System.Windows.Forms.MessageBoxIcon]::Warning
            ) | Out-Null
        ...
        [System.Windows.Forms.MessageBox]::Show(
            "Your workspace has newer files.`n`n$($checkResult.message)",
            'Update Available',
            [System.Windows.Forms.MessageBoxButtons]::OK,
            [System.Windows.Forms.MessageBoxIcon]::Information
        ) | Out-Null
...
'@
    ...
    function Prompt-And-RunUpdateFlow {
        ...
        # Check if in local workspace mode
        if ($u.mode -eq 'local-workspace') {
            Prompt-And-RunLocalWorkspaceUpdate
            return
        }

        # Fallback to GitHub mode (legacy)
        $update = $null
        try {
